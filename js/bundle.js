...
var _edge_detection = require("./edge_detection.js");
...
(async () => {
  ...
  const mask_canvas2 = await (0, _image_utils2.convertURLtoCanvas)(mask_url2);
  const mask_canvas3 = await (0, _image_utils2.convertURLtoCanvas)(mask_url3);
  ...
  (0, _image_utils.showCanvasToScreen)(edge_canvas2, "edge2", "edge");
  (0, _image_utils.showCanvasToScreen)(mask_canvas3, "mask3", "mask");
  (0, _image_utils.showCanvasToScreen)(edge_canvas3, "edge3", "edge");
  (0, _image_utils.showCanvasToScreen)(mask_canvas4, "mask4", "mask");
  (0, _image_utils.showCanvasToScreen)(edge_canvas4, "edge4", "edge");
  (0, _image_utils.showCanvasToScreen)(mask_canvas5, "mask5", "mask");
...
exports.detectEdgeMask = detectEdgeMask;
...
function detectEdgeMask(mask_image, line_color_black_amount, blur_strength, is_dashed = false, number_of_dashed_lines = 0, dashed_line_cut_width = 0) {
  ...
  const mask_canvas = (0, _image_utils.makeImageBlur)(mask_image, blur_strength);
  ...
  for (let src_y = 0; src_y < height; src_y++) {
    ...
    for (let src_x = 0; src_x < width; src_x++) {
      ...
      const src_around_alpha_arr = [mask_pixel_arr[src_idx_tl + 3], mask_pixel_arr[src_idx_tr + 3], mask_pixel_arr[src_idx_bl + 3], mask_pixel_arr[src_idx_br + 3]];
      const alpha_zero_count = src_around_alpha_arr.filter(alpha => alpha === 0).length;

      if (alpha_zero_count < 4) {
        const alpha_substract_arr = src_around_alpha_arr.map(alpha => Math.abs(mask_pixel_arr[src_idx_cc + 3] - alpha));
...
},{"./image_utils":4}],3:[function(require,module,exports){
"use strict";
...
exports.convertURLtoImgObj = exports.showCanvasToScreen = exports.makeImageBlur = exports.convertURLtoCanvas = void 0;

var _glfx = require("./glfx.js");

const convertURLtoCanvas = async url => {
...
exports.convertURLtoCanvas = convertURLtoCanvas;

const makeImageBlur = (image, blurPixelLength) => {
  const {
    width: w,
    height: h
  } = image;

  const glfx_canvas = _glfx.fx.canvas();
...
exports.makeImageBlur = makeImageBlur;

const showCanvasToScreen = (canvas, label, class_name) => {
...
exports.showCanvasToScreen = showCanvasToScreen;

const convertURLtoImgObj = async url => {
  const img = new Image();
  img.src = url;
  return new Promise((resolve, reject) => {
    img.addEventListener("load", function () {
...
exports.convertURLtoImgObj = convertURLtoImgObj;
